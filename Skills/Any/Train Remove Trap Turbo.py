...
class CircuitUI:
    ...
    def __init__(self, gump_id):
        ...
        self.skillbar = API.CreateGumpSimpleProgressBar(
            ...
            "#666666",
            "#0059FF",
            int(API.GetSkill("Remove Trap").Value),
            int(API.GetSkill("Remove Trap").Cap),
        )
    ...
    def updateText(
        ...
        if skilltime is not None:
            ...
            self.l_skill.SetText(
                "Time to Max: " + str(h) + "h " + str(m) + "m " + str(s) + "s"
            )
        if timeelapsed is not None:
            h = timeelapsed // 3600
            m = (timeelapsed % 3600) // 60
            s = timeelapsed % 60
            self.l_ttime.SetText(
                "Time Elapsed: " + str(h) + "h " + str(m) + "m " + str(s) + "s"
            )
        if (disarm_count and disarm_time) is not None:
            self.l_tdisarm.SetText(
                "Disarmed Trap #" + str(disarm_count) + " in " + str(disarm_time) + "s"
            )
        if mean is not None:
            self.l_mean.SetText(f"Avg disarm time: {mean:.2f}s")
        self.l_skillfraction.SetText(
            f"{float(API.GetSkill('Remove Trap').Value):.1f}"
            + "/"
            + str(API.GetSkill("Remove Trap").Cap)
        )
        self.skillbar.SetProgress(
            API.GetSkill("Remove Trap").Value, API.GetSkill("Remove Trap").Cap
        )
...
def play_game(gump_id, size, trap_serial, ui):
    ...
    for SafeCounter in range(0, 50):
        ...
        if solutionFitness > 0:
            if DEBUG:
                API.SysMsg(
                    "Found a match in solution database with "
                    + str(solutionFitness)
                    + "% match",
                    33,
                )
            ...
            if DEBUG:
                API.SysMsg("Try: " + dir_to_str(TryDirection), 431)
        ...
        if DEBUG:
            API.SysMsg(
                "Path: " + pathString + " | Next: " + dir_to_str(TryDirection), 438
            )
        ...
        if attempt == MoveResult.WrongTry:
            if DEBUG:
                API.SysMsg("Wrong: " + dir_to_str(TryDirection), 447)
...
def move_to(gump_id, direction, ui):
    ...
    for i in range(50):
        if API.InJournal("successfully disarm"):
            if DEBUG:
                API.SysMsg("Returned 0: Succesfully Disarmed")
            return 0  # Disarmed
        if API.InJournal("fail to disarm the trap"):
            if DEBUG:
                API.SysMsg("Returned 1: Wrong Try")
            return 1  # Wrong Try
        if bool(API.HasGump(gump_id)):
            if DEBUG:
                API.SysMsg("Returned 2: Valid Try")
            return 2  # Valid Try
        API.Pause(0.25)
    ...
    if DEBUG:
        API.SysMsg("Something went wrong. Line 484")
...
def calculate_path_fitness(size, path):
    ...
    if DEBUG:
        API.SysMsg(
            str(fitness) + "% match. Steps: " + dir_list_to_str(pathSolution), 91
        )
    ...
    while True:
        ...
        if gain_rate != 0:
            skill_time_remaining = (
                API.GetSkill("Remove Trap").Cap - current_skill
            ) / gain_rate
